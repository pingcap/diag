...
type uploadEmphasisHandler struct {
	c *bootstrap.ForesightConfig
}

func UploadEmphasis(c *bootstrap.ForesightConfig) http.Handler {
	return &uploadEmphasisHandler{c}
}

func (h *uploadEmphasisHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	fn.Wrap(h.uploadEmphasis).ServeHTTP(w, r)
}

func (h *uploadEmphasisHandler) uploadEmphasis(ctx context.Context, r *http.Request) (*utils.SimpleResponse, utils.StatusError) {
	uuid := mux.Vars(r)["id"]
